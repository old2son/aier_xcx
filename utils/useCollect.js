 ...
 */
export function useCollect({ list = [], idKey = 'specialId', collectKey = 'status', countKey = 'specialGood' }) {
	/**
	 * 初始化收藏状态（批量查）
	 */
	...
	async function toggleCollect(item) {
		...
		item[countKey] = originalStatus === 1 ? Math.max(0, originalCount - 1) : originalCount + 1;
