...
export default {
	...
	actions: {
		/* 注册帐号 || 登录帐号 */
		verificationCodeLogin({ commit }, verificationCodeLoginParams) {
		...
		/* 手机号快捷登录 */
		phoneQuickLogin({ commit }, quickLoginParams) {
			return new Promise((resolve, reject) => {
				miniProgramLogin(quickLoginParams)
					.then((resp) => {
						if (resp.code === 200 && resp.data && resp.data.userToken) {
							uni.setStorageSync('token', resp.data.userToken);
							uni.setStorageSync('userInfo', resp.data);
							commit('setToken', resp.data.userToken);
							commit('setUserInfo', resp.data);
		...
		// 查询小程序隐私授权
		queryWeChatAppletPrivacyAuth({ commit }) {
			return new Promise((resolve, reject) => {
