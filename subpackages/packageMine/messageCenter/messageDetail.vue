      ...
      <view class="details-content">
        ...
        <view class="content-box">
          ...
          <template v-if="msgDetail.messageType === 1">
            <view class="content-title title-1">感谢您预约{{ msgDetail.activityLocation }}的参观游览。</view>
          ...
          <template v-else-if="msgDetail.messageType === 2">
            ...
            <view class="content-init margin-title">
              ⏰ 时间：{{ msgDetail.activityTime }}
            </view>
            <view class="content-init margin-title">
              📍 地点：{{ msgDetail.activityLocation }}
            </view>
          ...
          <template v-else-if="msgDetail.messageType === 3">
            <view class="content-init title-2 explain">
              根据国家法定节假日安排，为方便大家参观，本基地原定于{{ formatDate(msgDetail.closeTime) }}的例行闭馆日，将顺延至{{ msgDetail.adjustTime }}。
            </view>
            <view class="content-init margin-title">{{ formatDate(msgDetail.activityTime) }}：**正常开放** 🎉</view>
            <view class="content-init margin-title">{{ formatDate(msgDetail.closeTime) }}：**全天闭馆** 🔒</view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getTitle(type) {
        switch(type) {
          case 1: return '参观预约成功'
          case 2: return '活动即将开始！'
          case 3: return '闭馆通知'
          default: return ''
        }
      },
