	...
	<view class="feedback-container">
		...
		<view class="question-content" :style="scrollViewStyle">
			<view class="question-title">
				...
				本问卷仅需1-2分钟，所有数据将严格保密。您的反馈对我们非常重要，感谢您的支持。
			</view>
					...
					<view class="options">
						...
						>
							<view
								...
								:class="{ 'nowrap-label': hasOtherOption(item) && sub === item.list.length - 1 }"
							>
								{{ opt.topicName }}
							</view>
							<van-field
			...
			<template v-if="listRendered">
				...
				<view class="last-q">
					...
					<van-field :value="lastTextAnswer" @input="lastTextAnswer = $event.detail" />
				</view>
...
export default {
	...
	methods: {
		...
		onSingleChoiceChange(index, val) {
			...
			const hasOther = this.queueList
				.find((i) => i.value === index)
				?.list.find((opt) => opt.topicName.includes('其他'));
			if (hasOther) {
...
.feedback-container {
	width: 100%;
	height: 100vh;
	background: linear-gradient(to bottom, #f9f9f9 0%, #f9f9f9 35%, #fff 50%, #fff 100%);
	overflow-y: auto;
