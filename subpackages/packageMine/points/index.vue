<template>
  <view class="points-container">
    ...
    <view 
      ...
      <view class="header-1">
        <text>我的积分</text>
        <image :src="pointsData.goPoints" mode="widthFix" @touchstart="toSubpackagePage('/subpackages/packageMine/points/pointsRecord')"></image>
      </view>
      <view class="header-2">
        <text class="point-num">{{ pointsResult.points }}</text>
        <text class="point-record" @click="toSubpackagePage('/subpackages/packageMine/points/exchangeRecord')">兑换记录</text>
      </view>
      ...
      <view class="sign-in-box">
        <view class="streak-info">
          <text>已连续签到</text> 
          <text class="day-color">{{ signDays }}</text>天
        </view>
        <view class="days">
        ...
        <button
          :class="['sign-btn', signButtonClass]"
...
export default {
  ...
  methods: {
    ...
    getSignIcon(index) {
      ...
      const unsignedIcon = 'https://geducloud0617.oss-cn-shenzhen.aliyuncs.com/aier-applet/%E5%8F%AF%E7%AD%BE%E5%88%B0.png'
      // 已签到的前 N 天用已签到图标
