<template>
	<view class="collect-container">
		<CustomNavInner title="我的收藏" />
		<view class="collect-list" :style="{ top: menuInfo.menuHeight + menuInfo.menuHeightFromTop + 20 + 'px' }">
			<view v-if="collectList.length === 0" class="empty-box">
				<image :src="empty" class="empty-image" mode="widthFix"></image>
				<view class="empty-text">收藏为空，快去收藏作品吧~</view>
			</view>
			<view class="collect-item" v-for="(item, index) in collectList" :key="item.specialId" v-else>
				<view class="poster">
...
<script>
import { scienceMyCollect } from '@/api';
...
export default {
	...
	methods: {
		async getRecommendList() {
			const res = await scienceMyCollect();
