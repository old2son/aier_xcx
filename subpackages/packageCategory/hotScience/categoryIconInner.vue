<template>
	<view class="category-icon-inner-container">
		<CustomNavInner title="爱尔眼健康科普教育基地" />
		<view class="screen-wrap" :style="{ top: menuInfo.menuHeight + menuInfo.menuHeightFromTop + 20 + 'px' }">
			<van-tabs
				v-model="active"
			...
			>
				<van-tab v-for="(item, index) in filterList" :title="item.pcExtensionUrl" :key="item.companyId">
...
<script>
import { getOrbitopathyYearQuery, getOrbitopathyQueryReviewVideo } from '@/api';
...
.screen-wrap {
	...
	.video-screen-wrap .van-tabs__nav--card .van-tab.van-tab--active {
		width: fit-content;
		flex: initial !important;
		flex-shrink: 0 !important;
	}
